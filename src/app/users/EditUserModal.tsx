...
const EditUserModal: React.FC<EditUserModalProps> = ({
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    // Validações (mantenha as mesmas que no AddUserModal)
  
    try {
      setIsLoading(true);
      
      // Prepara os dados para envio - diferente do AddUserModal
      const userPayload = {
        ...userFormData,
        // Só faz hash da senha se foi alterada (campo preenchido)
        passwordHash: userFormData.passwordHash 
          ? await bcrypt.hash(userFormData.passwordHash, 10)
          : undefined
      };
  
      // Remove passwordHash se estiver vazio (para não sobrescrever a senha atual)
      if (!userFormData.passwordHash) {
      ...
      // Garante que todos os campos sejam retornados, mesclando os dados antigos com os novos
      const completeUserData = {
        ...user, // Dados originais do usuário
        ...updatedUser, // Dados retornados pela API
